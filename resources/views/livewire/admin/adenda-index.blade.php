<div class="card">
    <div class="card-header">
            <div class="px-6 py-1 flex items-center">
                <x-input class="form-control flex-1" placeholder="Ingrese el nombre de una adenda" type="text" wire:model="search"></x-input>
            </div>
        </div>
                        ...
                        <tbody>
                            @foreach ($adendas as $adenda)
                                <tr>
                                    <td>{{$adenda->id}}</td>
                                    <td>{{$adenda->objeto}}</td>
                                    <td>{{$adenda->fechainicio}}</td>
                                    <td>{{$adenda->fechafinalizacion}}</td>
                                    <td>{{$adenda->numeroresolucion}}</td>
                                    <td>{{$adenda->anioresolucion}}</td>
                                    <td>{{$adenda->areageneradora->name}}</td>
                                    <td>{{$adenda->arearesponsable->name}}</td>
                                    <td width="10px">
                                        <a class="btn btn-secondary btn-sm" target="blanck" href="{{Storage::url($adenda->file->url)}}">
                                            <i class="fa fa-file-pdf"></i>
                                        </a>
                                    </td>
                                    @can('admin.contratos.edit')
                                        <td width="10px">
                                            <a class="btn btn-primary btn-sm" href="{{route('admin.contratos.edit',$adenda)}}">
                                                <i class="fa fa-pen"></i>
                                            </a>
                                        </td>
                                    @endcan
                                    @can('admin.adendas.destroy')
                                        <td width="10px">
                                            <form action="{{route('admin.adendas.destroy',$adenda)}}" method="post">
                                                @csrf
                                                @method('delete')
